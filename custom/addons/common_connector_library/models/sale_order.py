...
class SaleOrder(models.Model):
    ...
    def validate_order_ept(self):
        ...
        # MOD : Edit the function to validate delivery too
        for picking in self.picking_ids:
            delivery = self.env['stock.picking'].search([('id', '=', picking.id)])


            #MANAGE DELIVERY QTY By L
            for line in delivery.move_ids_without_package:
                ...
                stock_quant_model = self.env['stock.quant'].search([('location_id', '=', code+'/Stock'), ('product_id', '=', line.product_id.id)])

                # give the quantity_done value of quantity demande so it doesn't raise an error
                    #Product available
                ...
                if stock_quant_model.quantity < line.product_uom_qty:
                    ...
                    picking.sale_id.backorder_exist = True
                    picking.sale_id.process = 'backorders'


            _logger.info("OKEY DOKEY YOU Setting quantity + Validate delivery ...")
    ...
    def process_orders_and_invoices_ept(self):
        ...
        for order in self:
            ...
            if work_flow_process_record.validate_order:
                ...
                #ADD By L
                # If There is One Delivery and it's not valid that means All Products are Not available so Set Order status to 'Nothing deliverable'
                delivery_invalid = self.env['stock.picking'].search([('sale_id', '=', order.id), ('state', '!=', 'done')])
                if delivery_invalid and len(order.picking_ids) == 1:
                    _logger.info("OKEY DOKEY YOU Setting Order Process to : Nothing Deliverable ...")
                    order.process = 'nothing'
